...
import type { IDescription, IFormatArray, IResult } from "./utils";
...
function getMetaDatafromXML(description: IDescription): IResult {
    ...
    for (let j = 0; j < nodeFormats.length; j++) {
        if (!resultat.formatImage) {
            resultat.formatImage = FormatImage.find((elt) =>
                elt.format === nodeFormats[j]
            );
        }
        if (!resultat.formatArray) {
            resultat.formatArray = FormatArray.find((elt) =>
                elt.format === nodeFormats[j]
            );
        }
    }
    ...
    for (let m = 0; m < layerNodes.length && layerNode === null; m++) {
        if (layerNodes[m].querySelector("Name")!.textContent === layerName) {
            ...
            let fixedHeight: string | number = layerNode.getAttribute(
                "fixedHeight",
            )!;
            let fixedWidth: string | number = layerNode.getAttribute(
                "fixedWidth",
            )!;
            if (fixedHeight !== null) {
                ...
                resultat.heightMapHeight =
                    fixedHeight > 0 &&
                        fixedHeight < (resultat.heightMapHeight as number)
                        ? fixedHeight
                        : resultat.heightMapHeight;
                requestedSize.height = fixedHeight > 0
                    ? fixedHeight
            ...
            if (fixedWidth !== null) {
                ...
                resultat.heightMapWidth =
                    fixedWidth > 0 &&
                        fixedWidth < (resultat.heightMapWidth as number)
                        ? fixedWidth
                        : resultat.heightMapWidth;
                requestedSize.width = fixedWidth > 0
                    ? fixedWidth
                    : requestedSize.width;
            }
        }
    ...
    if (layerNode !== null && version !== null) {
        ...
        for (let n = 0; n < CRSTab.length && !found; n++) {
            ...
            if (nodeBBox !== null) {
                ...
                resultat.getTileDataAvailable = function (x, y, level) {
                    let retour = false;
                    let rectangleCalcul = resultat.tilingScheme!
        ...
        for (let q = 0; q < tileSets.length && !out; q++) {
            ...
            const isGoodLayer =
                tileSets[q].querySelector("Layers")!.textContent === layerName;
        ...
        resultat.ready = found &&
            (resultat.formatImage || resultat.formatArray) && version !== null;
    }

    if (resultat.ready) {
        let URLtemplate = url + "?SERVICE=WMS&REQUEST=GetMap&layers=" +
            layerName + "&version=" + version + "&bbox=";
        ...
        if (resultat.formatImage) {
            let URLtemplateImage = URLtemplate + "&format=" +
                resultat.formatImage.format + "&width=" + requestedSize.width +
                "&height=" + requestedSize.height;
            if (styleName) {
                URLtemplateImage += "&styles=" + styleName + "&style=" +
                    styleName;
            }
        ...
        if (resultat.formatArray) {
            const URLtemplateArray = URLtemplate + "&format=" +
                (resultat.formatArray as IFormatArray).format + "&width=" +
