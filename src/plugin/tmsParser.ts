...
function getMetaDatafromXML(description: IDescription): IResult {
    ...
    const selectedFormatImage = FormatImage.find((elt) =>
        elt.extension == format.getAttribute("extension")
    );
    ...
    if (resultat.formatImage) {
        tileSets = tilsetsNode.map(function (tileSet) {
            let url = tileSet.getAttribute("href") + "/{x}/{tmsY}." +
                resultat.formatImage!.extension;
            if (proxy) {
    ...
    if (
        tileSets.length === 0 || !resultat.formatImage || !resultat.tilingScheme
    ) {
    ...
    resultat.getTileDataAvailable = function (x, y, level) {
        ...
        return scratchRectangle !== null && level < maxLevel &&
            level < tileSets.length;
    };
    resultat.ready = true;
