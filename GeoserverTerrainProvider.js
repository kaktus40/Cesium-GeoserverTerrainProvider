(function() {
							...
							.otherwise(
									function() {
										...
										if (Cesium.defined(that.styleName)) {
											url += "&styles=" + that.styleName
													+ "&style="
													+ that.styleName
													+ '&format=image/png';
											var promiseZero = Cesium
	...
	/**
	 * become true after getMetaDatafromXML method didn't have problems during
	 * process.
	 * 
	 * @see WmsParserHelper.getMetaDatafromXML
	 */
	...
	Cesium.defineProperties(GeoserverTerrainProvider.prototype, {
		...
		tilingScheme : {
			get : function() {
				return this._wmsParserHelper.tilingScheme;
			}
		},
